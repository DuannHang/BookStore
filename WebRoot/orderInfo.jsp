										 ...
										 <c:forEach items="${order.orderItems }" var="oi" varStatus="vs">
												<table width="100%" border="0" cellspacing="0">
													<tr>
														...
														<td width="40%">${oi.p.name }</td>
														<td width="10%">${oi.p.price }</td>
														<td width="10%">${oi.buynum }</td>
														<td width="10%">${oi.buynum * oi.p.price }</td>

													</tr>
												</table>
									<c:set var="sum" value="${sum+oi.buynum * oi.p.price }"> </c:set>
										</c:forEach>
											...
											<p style="text-align: right">
												<a href="${pageContext.request.contextPath }/getOrderidAndSum?orderid=${order.id }&money=${sum }"><img src="images/gif53_029.gif"
